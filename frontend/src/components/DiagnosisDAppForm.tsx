...
const INPUTBOX_ABI = [
  "function addInput(address _dapp, bytes memory _input) returns (bytes32)",
];
...
export const DiagnosisDAppForm: React.FC = () => {
  ...
  const handleRegisterOnChain = async (item: HistoryItem) => {
    ...
    try {
      ...
      setToast({
        type: "info",
        message: `Enviando Input para DApp ${DAPP_ADDRESS.substring(0, 15)}...`,
      });
      ...
      setToast({
        type: "error",
        message: "Erro ao registrar: " + (error.reason || error.message),
      });
    } finally {
      setSendingId(null);
    }
    ...
    <div className="container">
      <div className="form-section" style={{ maxWidth: "1000px" }}>
        ...
        >
          ...
          {!walletAddress ? (
            <button
              ...
              style={{ background: "#f39c12", padding: "8px 16px" }}
            >
              🦊 Conectar MetaMask
            </button>
          ) : (
        ...
        <p style={{ color: "#aaa", marginBottom: "30px", lineHeight: "1.5" }}>
          ...
          camada de execução verificável (Cartesi Machine). Isso cria uma prova
            ...
            {history.length === 0 ? (
              ...
              >
                ...
                <small style={{ color: "#666" }}>
                  Realize um diagnóstico nas outras abas primeiro.
                </small>
              </div>
            ) : (
              <table
