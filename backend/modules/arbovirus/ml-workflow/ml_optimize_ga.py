...
# Se você estiver rodando fora do Docker ou quiser isolar os artefatos 
# no módulo de arbovírus, você pode alterar esse caminho no futuro.
...
def load_data():
    ...
    try:
        ...
        if os.path.exists(cols_path):
            ...
            df = pd.read_sql(
                f"SELECT {cols_str} FROM cleaned_arboviroses_cases", engine
            )
        else:
            ...
            feature_cols = [
                ...
                for c in df.columns
                if c
                not in ["doenca_alvo", "target_encoded", "dt_notific", "id_municip"]
            ]

        total_rows = len(df)
        if total_rows > LIMIT_FOR_GA:
            print(
                f"Dataset grande ({total_rows} linhas). Usando amostra de {LIMIT_FOR_GA} para o AG."
            )
...
def run_genetic_algorithm():
    ...
    # Certifique-se de que a pasta de artefatos existe antes de salvar
