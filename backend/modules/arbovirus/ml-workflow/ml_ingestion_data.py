...
def buscar_dados_paginados(
    disease_name, disease_url, limit_por_pagina=20, num_paginas=10
):
    ...
    print(
        f"Busca para {disease_name} finalizada. Total de {len(df_completo)} registros coletados."
    )
    return df_completo
...
def main():
    ...
    for nome, url in doencas_endpoints.items():
        dados_doenca = buscar_dados_paginados(
            nome, url, limit_por_pagina=20, num_paginas=500
        )
    ...
    if not todos_os_dados:
        ...
        try:
            ...
            df_final_bruto.to_sql(
                "raw_arboviroses_cases", engine, if_exists="replace", index=False
            )
