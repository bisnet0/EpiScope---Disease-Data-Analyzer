...
from backend.modules.womens_health.models.womens_health_profile import db, WomensHealthProfile
...
from backend.modules.integrations.services.google_fit_service import get_google_fit_data_expanded
...
def update_cycle_data_service(user_id: str, data: dict):
    ...
    try:
        ...
        db.session.commit()
        return {"message": "Perfil ginecológico atualizado", "profile": profile.to_dict()}, 200
...
def _get_average_heart_rate(user_id: str):
    ...
    if recent_hr == 0:
        try:
            ...
            recent_activities = StravaActivity.query.filter_by(user_id=user_id, has_heartrate=True)\
                                              .order_by(StravaActivity.start_date.desc()).limit(5).all()
            if recent_activities:
                total_hr = sum(act.average_heartrate for act in recent_activities if act.average_heartrate)
...
def analyze_cycle_with_wearables(user_id: str):
    ...
    if not profile.last_period_start:
        return {"status": "pending", "message": "Necessário configurar a data da última menstruação para prever o ciclo."}, 200

    # 1. Cálculos Baseados no Calendário
    today = datetime.utcnow().date()
    days_since_period = (today - profile.last_period_start).days
    
    # Se já passou muito tempo do ciclo médio, significa que ela está atrasada
    cycle_length = profile.average_cycle_length
    if days_since_period > cycle_length + 5:
        return {
            "status": "alert",
            "message": "Seu ciclo está atrasado. Atualize a data da última menstruação se ela já ocorreu.",
            "current_day_of_cycle": days_since_period,
            "clinical_insights": ["Atraso menstrual detectado. Se houver possibilidade de gravidez, considere um teste."]
        }, 200
    ...
    if recent_hr > 0:
        ...
        if current_phase == "Lútea":
            insights.append("Nota: Durante a fase lútea, o aumento da progesterona pode elevar a frequência cardíaca de repouso em 2 a 5 BPM.")
        elif current_phase == "Ovulatória (Janela Fértil)":
            insights.append("Picos ligeiros na Frequência Cardíaca de Repouso (RHR) são comuns agora e ajudam a confirmar a ovulação.")
    else:
        insights.append("Conecte seu Google Fit ou Strava para obter previsões cruzadas com sua Frequência Cardíaca.")

    if profile.is_perimenopause:
        insights.append("Perimenopausa ativa: Oscilações hormonais podem causar flutuações atípicas no ciclo e na frequência cardíaca (palpitações).")
    ...
    analysis_result = {
        "status": "success",
        "current_day_of_cycle": days_since_period + 1,
