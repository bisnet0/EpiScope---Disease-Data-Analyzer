...
try:
    ...
    idade_series = pd.read_sql_query(
        'SELECT "nu_idade_n" FROM raw_arboviroses_cases', engine
    )
    global_idade_median = pd.to_numeric(
        idade_series["nu_idade_n"], errors="coerce"
    ).median()
    ...
    is_first_chunk = {
        TEMP_TABLE_DENGUE: True,
        TEMP_TABLE_ZIKA: True,
        TEMP_TABLE_CHIK: True,
    }
    ...
    for i, df_raw in enumerate(reader):
        ...
        if not df_dengue.empty:
            df_dengue.to_sql(
                TEMP_TABLE_DENGUE,
                engine,
                if_exists="replace" if is_first_chunk[TEMP_TABLE_DENGUE] else "append",
                index=False,
            )
        ...
        if not df_zika.empty:
            df_zika.to_sql(
                TEMP_TABLE_ZIKA,
                engine,
                if_exists="replace" if is_first_chunk[TEMP_TABLE_ZIKA] else "append",
                index=False,
            )
        ...
        if not df_chik.empty:
            df_chik.to_sql(
                TEMP_TABLE_CHIK,
                engine,
                if_exists="replace" if is_first_chunk[TEMP_TABLE_CHIK] else "append",
                index=False,
            )
    ...
    count_dengue = pd.read_sql(
        f"SELECT COUNT(*) FROM {TEMP_TABLE_DENGUE}", engine
    ).iloc[0, 0]
    count_zika = pd.read_sql(f"SELECT COUNT(*) FROM {TEMP_TABLE_ZIKA}", engine).iloc[
        0, 0
    ]
    count_chik = pd.read_sql(f"SELECT COUNT(*) FROM {TEMP_TABLE_CHIK}", engine).iloc[
        0, 0
    ]
    ...
    print(
        f"Contagens: Dengue={count_dengue}, Zika={count_zika}, Chikungunya={count_chik}"
    )
    ...
    df_dengue_balanced = pd.read_sql(
        f"SELECT * FROM {TEMP_TABLE_DENGUE} ORDER BY RANDOM() LIMIT {min_count}", engine
    )
    df_zika_balanced = pd.read_sql(
        f"SELECT * FROM {TEMP_TABLE_ZIKA} ORDER BY RANDOM() LIMIT {min_count}", engine
    )
    df_chik_balanced = pd.read_sql(
        f"SELECT * FROM {TEMP_TABLE_CHIK} ORDER BY RANDOM() LIMIT {min_count}", engine
    )

    print("Concatenando dataset final balanceado...")
    df_final_balanced = (
        pd.concat([df_dengue_balanced, df_zika_balanced, df_chik_balanced])
        .sample(frac=1)
        .reset_index(drop=True)
    )

    print(
        f"Salvando tabela final '{FINAL_TABLE}' com {len(df_final_balanced)} registros..."
    )
