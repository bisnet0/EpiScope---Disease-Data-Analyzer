...
from sklearn.metrics import classification_report # <--- NOVO
from sqlalchemy import create_engine, text # <--- NOVO
from dotenv import load_dotenv # <--- NOVO
import json
import time

load_dotenv() # Carrega variáveis de ambiente para o DB
...
# Parâmetros (Agrupados para salvar no log depois)
TRAIN_PARAMS = {
    "img_size": 224,
    "batch_size": 16,
    "initial_epochs": 20,
...
def preprocess_image(image_path, target_size=(TRAIN_PARAMS["img_size"], TRAIN_PARAMS["img_size"])):
    try:
...
try:
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        images, encoded_labels, test_size=TRAIN_PARAMS["test_split"], random_state=42, stratify=encoded_labels
    )
    ...
    train_generator = train_datagen.flow(X_train, y_train, batch_size=TRAIN_PARAMS["batch_size"])
    val_generator = val_datagen.flow(X_val, y_val, batch_size=TRAIN_PARAMS["batch_size"])

    # 5. Construir Modelo
    base_model = MobileNetV2(weights='imagenet', include_top=False, input_shape=(TRAIN_PARAMS["img_size"], TRAIN_PARAMS["img_size"], 3))
    ...
    # 9. Avaliar e Gerar Relatório
    print("Avaliando modelo final...")
    loss, accuracy = model.evaluate(val_generator, steps=max(1, len(X_val) // TRAIN_PARAMS["batch_size"]))
    
    # Gerar predições para o Classification Report
    # Importante: Usamos X_val direto para garantir a ordem correta (generators podem embaralhar)
    ...
    print("\n--- Relatório (CNN): ---")
    print(classification_report(y_val, y_pred_class, target_names=label_encoder.classes_))
    ...
    try:
        log_entry_orm = {
            ...
            "feature_importance": None, # CNN não tem feature importance tabular
            "metrics": report_dict,
            "accuracy": accuracy,
            "dataset_size": len(images),
            "created_at": pd.Timestamp.utcnow() # ou datetime.now()
        }
        ...
        with engine.connect() as conn:
            conn.execute(insert_query, {
                ...
                "feature_importance": None, # Envia null para o banco
